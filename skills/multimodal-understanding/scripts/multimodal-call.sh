...
GATEWAY_URL="https://llm-gateway-api.nodesk.tech"
...
if [ -z "$API_KEY" ]; then
    echo '{"error": "API key not found in ~/.deskclaw/nanobot/config.json (checked providers.custom, anthropic, openai)"}'
    exit 1
fi
...
send_request() {
    curl -sL --max-time 120 \
        ...
        "${GATEWAY_URL}/deskclaw/v1/multimodal/" \
        -d @"$1" 2>&1
}
